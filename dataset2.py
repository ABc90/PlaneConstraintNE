...
def load_shape(point_filename, normals_filename, curv_filename, pidx_filename):
    ...
    if normals_filename != None:
        ...
        # curvatures = np.load(curv_filename+'.npy')  ####jun
...
class PointcloudPatchDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        Delta = torch.FloatTensor(self.points_per_patch*len(self.patch_radius_absolute[shape_ind])).zero_()
        Hist = torch.FloatTensor(10).zero_()  # bin=10
        Curv =  torch.FloatTensor(self.points_per_patch*len(self.patch_radius_absolute[shape_ind]), 2).zero_()
        ...
        for s, rad in enumerate(self.patch_radius_absolute[shape_ind]):
            ...
            nnn = int(np.ceil(self.points_per_patch / len(patch_point_inds)) - 1)
            if nnn >= 1:
                patch_point_inds = np.tile(patch_point_inds, nnn + 2)
                patch_point_inds = patch_point_inds[:self.points_per_patch]
            ...
            if self.include_normals:
                ...
                patch_wisenormal = torch.from_numpy(shape.normals[patch_point_inds,:])
                delta_mormal = torch.min((patch_wisenormal - patch_normal).pow(2).sum(1).sqrt(), (patch_wisenormal + patch_normal).pow(2).sum(1).sqrt())
                delta_mormal = (delta_mormal-torch.min(delta_mormal))/(torch.max(delta_mormal)-torch.min(delta_mormal)+0.01)
                Delta[start:end] = delta_mormal
        ...
        if self.include_normals:
            ...
            # hist???
            nn = np.multiply(patch_normal.view(1, 3).repeat(len(patch_point_inds), 1), patch_wisenormal)
            nn = torch.sum(nn, dim=1)
            nn[nn>1] = 1
            nn[nn<-1]=-1
            patch_wisenormal_deg = np.rad2deg(np.arccos(np.abs(nn)))
            max_v = torch.max(patch_wisenormal_deg)
            min_v = torch.min(patch_wisenormal_deg)
            patch_wisenormal_deg[patch_wisenormal_deg>max_v]=max_v
            patch_wisenormal_deg[patch_wisenormal_deg<min_v]=min_v
            hist, _ = np.histogram(patch_wisenormal_deg, 15, (min_v.cpu().numpy(),max_v.cpu().numpy()))

            # delta_mormal = torch.min((patch_wisenormal - patch_normal).pow(2).sum(1).sqrt(), (patch_wisenormal + patch_normal).pow(2).sum(1).sqrt())
            # delta_mormal = (delta_mormal-torch.min(delta_mormal))/(torch.max(delta_mormal)-torch.min(delta_mormal)+0.01)
            # Delta[start:end] = delta_mormal
            Hist = torch.from_numpy(hist.copy()).float()




        if self.include_curvatures:
            patch_curvs = torch.from_numpy(shape.curv[patch_point_inds,:])
            # scale curvature to match the scaled vertices (curvature*s matches position/s):
            patch_curvs = patch_curvs * self.patch_radius_absolute[shape_ind][0]
            Curv[start:end,:] = patch_curvs
        ...
        return (patch_pts,) + patch_feats + (trans,) + (Delta,) + (Hist,) + (Curv,)
    ...
    def load_shape_by_index(self, shape_ind):
        ...
        # curv_filename = os.path.join(self.root, self.shape_names[shape_ind] + '.curv')              #junz
