...
evaluate_results_root_nesti_net = '/Users/jinwei/Downloads/all_model_logs/experts/pcpnet_results/';
evaluate_results_root_pcpnet = '/Users/jinwei/Downloads/data_ms/pcpnet/pclouds_results/multi_scale_normal/';
...
%     end
    ...
    normals_result_name_pcpnet = [evaluate_results_root_pcpnet current_name '.normals'];
    normals_result_name_nesti_net = [evaluate_results_root_nesti_net current_name '.normals'];
...
%     end
    ...
    normals_result_nesti_net = load(normals_result_name_nesti_net);
    normals_result_pcpnet = load(normals_result_name_pcpnet);
    points_gt_pc = pointCloud(points_gt);
    normals_result_pca = pcnormals(points_gt_pc,18*20);
...
%     ang_o = rad2deg(acos(nn));
    ...
    diff_nesti_net = abs(sum(normals_result_nesti_net.*normals_gt,2))./ (sqrt(sum(normals_result_nesti_net.^2,2)).* sqrt(sum(normals_gt.^2,2)));
    diff_nesti_net(diff_nesti_net > 1) = 1;
    ang_nesti_net = acosd(diff_nesti_net);
    
    diff_pcpnet = abs(sum(normals_result_pcpnet.*normals_gt,2))./ (sqrt(sum(normals_result_pcpnet.^2,2)).* sqrt(sum(normals_gt.^2,2)));
    diff_pcpnet(diff_pcpnet > 1) = 1;
    ang_pcpnet = acosd(diff_pcpnet);
    
    diff_pca= abs(sum(normals_result_pca.*normals_gt,2))./ (sqrt(sum(normals_result_pca.^2,2)).* sqrt(sum(normals_gt.^2,2)));
    diff_pca(diff_pca > 1) = 1;
    ang_pca = acosd(diff_pca);
    %% hist 
    h1 = [h1;ang];
    h2 = [h2;ang_nesti_net];
    h3 = [h3;ang_pcpnet];
...
%% rms_ang
    ...
    rms_ang_nesti_net = sqrt(mean(ang_nesti_net.^2));
    rms_ang_pcpnet = sqrt(mean(ang_pcpnet.^2));
    rms_ang_pca = sqrt(mean(ang_pca.^2));
    
    
    Total= Total+rms_ang;
    Total_nesti_net= Total_nesti_net+rms_ang_nesti_net;
    Total_pcpnet= Total_pcpnet+rms_ang_pcpnet;
    Total_pca= Total_pca+rms_ang_pca;
...
disp(Total_nesti_net/length(test_name));
disp(Total_pcpnet/length(test_name));
disp(Total_pca/length(test_name));
...
x = [0 5 10 20 40 60 90];
h = hist([h1,h2,h3,h4],x); 
bar(h);
