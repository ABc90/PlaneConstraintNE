...
%         count = count+1
%         abs_radius = 0.05*bbdiag;
%         center = points_gt(iii,:);
%         [idx,D] = rangesearch(points_gt,center,abs_radius);
%         patch = points_gt(idx{1:end},:);
%         patch = (patch-center)/abs_radius;
%         patch = patch./sqrt(sum((patch).^2,2)+1e-10);
%         gt = normals_gt(iii,:)./sqrt(sum((normals_gt(iii,:)).^2,2)+1e-10);
%         gt_part = normals_gt(idx{1:end},:)./sqrt(sum((normals_gt(idx{1:end},:)).^2,2)+1e-10);
%         mask =min([sqrt(sum((gt-gt_part).^2,2)),sqrt(sum((gt+gt_part).^2,2))],[],2);
%         mask = mask/max(mask);
%         mask(mask>0.6)=1;
%         mask(mask<=0.6)=0;
%         mask = 1-mask;
%         gt1 = normals_result1(iii,:)./sqrt(sum((normals_result1(iii,:)).^2,2)+1e-10);
%         gt2 = normals_result2(iii,:)./sqrt(sum((normals_result2(iii,:)).^2,2)+1e-10);
%         gt3 = normals_result3(iii,:)./sqrt(sum((normals_result3(iii,:)).^2,2)+1e-10);
%         a = mean(abs(sum(patch.*repelem(gt,size(patch,1),1),2)));
%         a1 = mean(abs(sum(patch.*repelem(gt1,size(patch,1),1),2)));
%         a2 = mean(abs(sum(patch.*repelem(gt2,size(patch,1),1),2)));
%         a3 = mean(abs(sum(patch.*repelem(gt3,size(patch,1),1),2)));
%         disp([a1,a2,a3,a]);
%         disp([ang1(iii),ang2(iii),ang3(iii),0]);
%         [v,index] = min([a1,a2,a3],[],2);
